function gssh --description "Connect to GCP instance over SSH (in dev env by default)"
    ...
    if test "$env" = "backbone"
        set -l ip (gcloud compute instances describe --project=$project --zone=$zone --format="value(networkInterfaces[0].accessConfigs[0].natIP)" $host)
        set -l ssh_opts ""
    else
        set -l ip (gcloud compute instances describe --project=$project --zone=$zone --format="value(networkInterfaces[0].networkIP)" $host)
        set -l ssh_opts "-o StrictHostKeyChecking=no -o ProxyCommand='ssh -W %h:%p $env'"
    end
